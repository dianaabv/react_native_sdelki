...
class SegmentNB extends Component {
  ...
  const formData = `deal616=${JSON.stringify(this.state.deal616)}&deal_id=${this.state.deal.deal_id}&duedate=${this.state.duedate}&workdeadline=${this.state.workdeadline}&lawid=${this.state.lawid}`
...
<Text style={styles.myFont1}>Описание работ</Text>
  ...
  <TextInput style={[ !this.state.isRegular? styles.myInputBorder : styles.myInput]} numberOfLines = {3} editable={!this.state.isRegular}
    multiline={true}
   onChangeText={(text) => this.deal616(text, 'workdescription')}
  defaultValue={this.state.deal.workdescription}
  placeholder={this.state.deal.workdescription}  />

<Text style={styles.myFont1}>Адрес объекта при проведении работ по ремонту</Text>

  <TextInput style={[ !this.state.isRegular? styles.myInputBorder : styles.myInput]} numberOfLines = {3} editable={!this.state.isRegular}
    multiline={true}
  onChangeText={(text) => this.deal616(text, 'workaddress')}
  defaultValue={this.state.deal.workaddress}
  placeholder={this.state.deal.workaddress}  />

<Text style={styles.myFont1}>Срок выполнения работ</Text>
<Item disabled regular={!this.state.isRegular}>
  <Input disabled

  placeholder={this.dateFormat(this.state.deal.workdeadline)}  />
  {(this.state.toChange)?(
    <DatePicker mode="date" format="YYYY-MM-DD"
    confirmBtnText="Confirm"
    cancelBtnText="Cancel"
    onDateChange={(date) => {this.setState({workdeadline: date})}}
    />
    ):(<Text></Text>)}
</Item>
<Text style={styles.myFont1}>Цена работ</Text>
  ...
  <TextInput style={[ !this.state.isRegular? styles.myInputBorder : styles.myInput]} numberOfLines = {3} editable={!this.state.isRegular}
    multiline={true}
  onChangeText={(text) => this.deal616(text, 'workprice')}
  defaultValue={this.state.deal.workprice}
  placeholder={this.state.deal.workprice}  />

<Text style={styles.myFont1}>Сроки и порядок оплаты</Text>

  <TextInput style={[ !this.state.isRegular? styles.myInputBorder : styles.myInput]} numberOfLines = {3} editable={!this.state.isRegular}
    multiline={true}
  onChangeText={(text) => this.deal616(text, 'payday')}
...
<Text style={styles.myFont1}>Порядок приема работ</Text>
  ...
  <TextInput style={[ !this.state.isRegular? styles.myInputBorder : styles.myInput]} numberOfLines = {3} editable={!this.state.isRegular}
    multiline={true}
  onChangeText={(text) => this.deal616(text, 'workcheck')}
  defaultValue={this.state.deal.workcheck}
  placeholder={this.state.deal.workcheck}  />

<Text style={styles.myFont1}>Качество работ (гарантия качества работ)</Text>

  <TextInput style={[ !this.state.isRegular? styles.myInputBorder : styles.myInput]} numberOfLines = {3} editable={!this.state.isRegular}
    multiline={true}
  onChangeText={(text) => this.deal616(text, 'quantity')}
...
<Text style={styles.myFont1}>Дополнительные условия (не обязательное ус-ие)</Text>
  ...
  <TextInput style={[ !this.state.isRegular? styles.myInputBorder : styles.myInput]} numberOfLines = {3} editable={!this.state.isRegular}
    multiline={true}
  onChangeText={(text) => this.deal616(text, 'additional')}
...
{(this.state.toChange)?(<Button block onPress={() => this.updateDeal616()}><Text>Подтвердить изменения</Text></Button>):(
...
{(this.state.status=='acceptor' && this.state.acceptor_status=='requested_deny')?(<Button style={styles.myPadding}  block onPress={() => this.denyDeny()} ><Text>Отклонить запрос на расторжение сделки</Text></Button>):(<Text></Text>)}
  ...
  <Spinner color="blue" />
        ...
        {(Object.keys(this.state.olddeal).length != 0)?(
          ...
          <Item disabled   >
            <Input disabled
              multiline={true}
            defaultValue={this.state.olddeal.workdescription}
          ...
          <Item disabled   >
            ...
            {objKeys.includes("workaddress")? <IconNB name="md-create" />:<Text></Text>}
          </Item>
          <Text style={styles.myFont1}>Срок выполнения работ</Text>
          <Item disabled   >
            <Input disabled
  multiline={true}
            ...
            {objKeys.includes("workdeadline")? <IconNB name="md-create" />:<Text></Text>}
          </Item>
          <Text style={styles.myFont1}>Цена работ</Text>
          <Item disabled   >
            <Input disabled
  multiline={true}
            ...
            {objKeys.includes("workprice")? <IconNB name="md-create" />:<Text></Text>}
          </Item>
          <Text style={styles.myFont1}>Сроки и порядок оплаты</Text>
          <Item disabled   >
            <Input disabled
  ...
  multiline={true}
            ...
            {objKeys.includes("workcheck")? <IconNB name="md-create" />:<Text></Text>}
          </Item>
          <Text style={styles.myFont1}>Качество работ (гарантия качества работ)</Text>
          <Item disabled   >
            <Input disabled
