...
class SegmentNB extends Component {
  ...
  const formData = `deal540=${JSON.stringify(this.state.deal540)}&deal_id=${this.state.deal.deal_id}&duedate=${this.state.duedate}&lawid=${this.state.lawid}`
...
<Text style={styles.myFont1}>Наниматель</Text>
<Item disabled>
  <Input disabled
    multiline={true}
...
<Text style={styles.myFont1}>Наймодатель</Text>
<Item disabled>
  <Input disabled
    multiline={true}
...
<Text style={styles.myFont1}>Данные, позволяющие установить имущество, подлежащее передаче в аренду</Text>

  <TextInput style={[ !this.state.isRegular? styles.myInputBorder : styles.myInput]} numberOfLines = {3} editable={!this.state.isRegular}
    multiline={true}
   onChangeText={(text) => this.deal540(text, 'itemdata')}
  defaultValue={this.state.deal.itemdata}
  placeholder={this.state.deal.itemdata}  />

<Text style={styles.myFont1}>Cроки и порядок передачи/возврата имущества</Text>

  <TextInput style={[ !this.state.isRegular? styles.myInputBorder : styles.myInput]} numberOfLines = {3} editable={!this.state.isRegular}
    multiline={true}
  onChangeText={(text) => this.deal540(text, 'giveawaydeadline')}
...
<TextInput style={[ !this.state.isRegular? styles.myInputBorder : styles.myInput]} numberOfLines = {3} editable={!this.state.isRegular}
  multiline={true}
onChangeText={(text) => this.deal540(text, 'payday')}
defaultValue={this.state.deal.payday}
placeholder={this.state.deal.payday}  />

<Text style={styles.myFont1}>Условия о содержании/улучшении имущества</Text>

  <TextInput style={[ !this.state.isRegular? styles.myInputBorder : styles.myInput]} numberOfLines = {3} editable={!this.state.isRegular}
    multiline={true}
  onChangeText={(text) => this.deal540(text, 'keepcondition')}
  defaultValue={this.state.deal.keepcondition}
  placeholder={this.state.deal.keepcondition}  />

<Text style={styles.myFont1}>Условия об использовании имущества (в т.ч. пределах распоряжения)</Text>

  <TextInput style={[ !this.state.isRegular? styles.myInputBorder : styles.myInput]} numberOfLines = {3} editable={!this.state.isRegular}
    multiline={true}
  onChangeText={(text) => this.deal540(text, 'usecondition')}
...
<Text style={styles.myFont1}>Дополнительные условия (не обязательное ус-ие)</Text>
  ...
  <TextInput style={[ !this.state.isRegular? styles.myInputBorder : styles.myInput]} numberOfLines = {3} editable={!this.state.isRegular}
    multiline={true}
  onChangeText={(text) => this.deal540(text, 'additional')}
...
{(this.state.create_as_ip=='accept_as_ip')?(
  ...
  <Content>
  {(this.state.status=='acceptor' && this.state.acceptor_status=='requested')?(<Button block style={styles.myPadding} onPress={() => this.acceptDeal()}>
...
{(this.state.toChange)?(<Button block onPress={() => this.updateDeal540()}><Text>Подтвердить изменения</Text></Button>):(
  ...
  <Spinner color="blue" />
        ...
        {(Object.keys(this.state.olddeal).length != 0)?(
          ...
          <Item disabled>
            <Input disabled
              multiline={true}
            placeholder={this.state.olddeal.employee.firstname+ ' '+ this.state.olddeal.employee.lastname}  />
          </Item>
          <Text style={styles.myFont1}>Наймодатель</Text>
          <Item disabled>
            <Input disabled
              multiline={true}
            placeholder={this.state.olddeal.employer.firstname+ ' '+ this.state.olddeal.employer.lastname}  />
          </Item>
    ...
    return (
      <Container>
        ...
        <Content padder>
          {this.state.seg === 1 &&
            <Content>
            <H2 style={styles.myFont}>Договор имущественного найма (аренды)</H2>
