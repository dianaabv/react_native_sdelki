...
myNext(value){
	if(value.employer){
		...
		const formData = `deal683=${JSON.stringify(value)}&employer=${value.employer}&employee=${employee.sub}&duedate=${value.duedate}&deadline=${value.deadline}&lawid=${this.state.lawid}&status=${this.state.status}`;
		this.updateDeal(formData)
	} else{
		var employer = jwtDecode(this.state.myKey)
		const formData = `deal683=${JSON.stringify(value)}&employer=${employer.sub}&employee=${value.employee}&duedate=${value.duedate}&deadline=${value.deadline}&lawid=${this.state.lawid}&status=${this.state.status}`;
		this.updateDeal(formData)
	}
}
  onPress() {
		...
		return (
			<Container style={styles.container}>
				<Header>
					...
					<Body>
						<Text style={styles.metext}>Договор возмездного оказания услуг</Text>
					</Body>
					<Right />
				</Header>
        <Content  style={styles.mb10}>
        <Text>Предмет договора:</Text>
        <Text>Исполнитель обязуется оказать услуги (совершить определенные действия или осуществить определенную деятельность), а Заказчик обязуется оплатить эти услуги на условиях, указанных в настоящем договоре.</Text>
