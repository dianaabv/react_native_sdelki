...
const options = {
    ...
    fields: {
    itemname:{
      label: "Наименование дара (вещи или имущественного права (требование) либо освобождения от имущественной обязанности)"
    },
    quantity:{
      label: "Количество дара"
    },
    deadline:{
      label: "Сроки и порядок передачи дара (момент перехода права собственности)"
    },
...
class NHListSeparator extends Component {
  ...
  getType(value) {
    if (value.Role === 'presenter' && this.state.status1=='Физическое Лицо') {
      return t.struct({
        Role: Role,
        receiver: t.enums(this.state.kontr_ids),
        itemname: t.String,
        quantity: t.String,
        deadline: t.String,
    ...
    } else if (value.Role === 'receiver' && this.state.status1=='Физическое Лицо' ) {
      return t.struct({
        Role: Role,
        presenter: t.enums(this.state.kontr_ids),
        itemname: t.String,
        quantity: t.String,
        deadline: t.String,
    ...
    } else if (value.Role === 'presenter' && this.state.status1=='Индивидуальный предприниматель' ) {
      return t.struct({
        Role: Role,
        receiver: t.enums(this.state.kontr_ids),
        itemname: t.String,
    ...
    } else if (value.Role === 'receiver' && this.state.status1=='Индивидуальный предприниматель' ) {
      return t.struct({
        Role: Role,
        presenter: t.enums(this.state.kontr_ids),
        itemname: t.String,
...
myNext(value){
	if(value.presenter){
		...
		var receiver = jwtDecode(this.state.myKey)
		const formData = `deal506=${JSON.stringify(value)}&presenter=${value.presenter}&receiver=${receiver.sub}&duedate=${value.duedate}&lawid=${this.state.lawid}&status=${this.state.status}`;
		this.updateDeal(formData)
	} else{
		var presenter = jwtDecode(this.state.myKey)
		const formData = `deal506=${JSON.stringify(value)}&employer=${employer.sub}&receiver=${value.receiver}&duedate=${value.duedate}&lawid=${this.state.lawid}&status=${this.state.status}`;
		this.updateDeal(formData)
	}
}
  onPress() {
		...
		return (
			<Container style={styles.container}>
				<Header>
					...
					<Body>
						<Text style={styles.metext}>Договор Дарения</Text>
					</Body>
					<Right />
				</Header>
        <Content  style={styles.mb10}>
        <Text>Предмет договора:</Text>
        <Text>Даритель обязуется передать Одаряемому вещь в собственность либо имущественное право (требование) к себе или третьему лицу, либо освобождает или обязуется освободить ее от имущественной обязанности перед третьим лицом на условиях, указанных в настоящем договоре.</Text>
