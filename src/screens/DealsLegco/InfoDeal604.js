...
class SegmentNB extends Component {
  ...
  const formData = `deal604=${JSON.stringify(this.state.deal604)}&deal_id=${this.state.deal.deal_id}&duedate=${this.state.duedate}&usedeadline=${this.state.usedeadline}&lawid=${this.state.lawid}`
...
<Text style={styles.myFont1}>Ссудополучатель</Text>
<Item disabled>
  <Input disabled
  multiline={true}
...
<Text style={styles.myFont1}>Ссудодатель</Text>
<Item disabled>
  <Input disabled
  multiline={true}
  placeholder={this.state.deal.lender.firstname+ ' '+ this.state.deal.lender.lastname}  />
    <IconNB name="ios-information-circle" />
</Item>

<Text style={styles.myFont1}>Данные, позволяющие установить имущество, подлежащее передаче</Text>

  <TextInput style={[ !this.state.isRegular? styles.myInputBorder : styles.myInput]} numberOfLines = {3} editable={!this.state.isRegular}
  multiline={true}
   onChangeText={(text) => this.deal604(text, 'itemdata')}
  defaultValue={this.state.deal.itemdata}
  placeholder={this.state.deal.itemdata}  />

<Text style={styles.myFont1}>Сроки и порядок передачи имущества</Text>

  <TextInput style={[ !this.state.isRegular? styles.myInputBorder : styles.myInput]} numberOfLines = {3} editable={!this.state.isRegular}
  multiline={true}
  onChangeText={(text) => this.deal604(text, 'giveawaydeadline')}
  defaultValue={this.state.deal.giveawaydeadline}
  placeholder={this.state.deal.giveawaydeadline}  />

<Text style={styles.myFont1}>Срок пользования имуществом</Text>
<Item disabled regular={!this.state.isRegular}>
  <Input disabled

  placeholder={this.dateFormat(this.state.deal.usedeadline)}  />
  {(this.state.toChange)?(
    <DatePicker mode="date" format="YYYY-MM-DD"
    confirmBtnText="Confirm"
    cancelBtnText="Cancel"
    onDateChange={(date) => {this.setState({usedeadline: date})}}
    />
    ):(<Text></Text>)}
</Item>
<Text style={styles.myFont1}>Условия о содержании имущества</Text>

  <TextInput style={[ !this.state.isRegular? styles.myInputBorder : styles.myInput]} numberOfLines = {3} editable={!this.state.isRegular}
  multiline={true}
  onChangeText={(text) => this.deal604(text, 'keepcondition')}
  defaultValue={this.state.deal.keepcondition}
  placeholder={this.state.deal.keepcondition}  />

<Text style={styles.myFont1}>Условия об использовании имущества</Text>

  <TextInput style={[ !this.state.isRegular? styles.myInputBorder : styles.myInput]} numberOfLines = {3} editable={!this.state.isRegular}
  multiline={true}
  onChangeText={(text) => this.deal604(text, 'usecondition')}
  defaultValue={this.state.deal.usecondition}
  placeholder={this.state.deal.usecondition}  />
...
<Text style={styles.myFont1}>Дополнительные условия (не обязательное ус-ие)</Text>
  ...
  <TextInput style={[ !this.state.isRegular? styles.myInputBorder : styles.myInput]} numberOfLines = {3} editable={!this.state.isRegular}
  multiline={true}
  onChangeText={(text) => this.deal604(text, 'additional')}
...
{(this.state.toChange)?(<Button block onPress={() => this.updateDeal604()}><Text>Подтвердить изменения</Text></Button>):(
  ...
  <Spinner color="blue" />
        ...
        {(Object.keys(this.state.olddeal).length != 0)?(
          ...
          <Item disabled>
            <Input disabled
            multiline={true}
            placeholder={this.state.olddeal.lender.firstname+ ' '+ this.state.olddeal.lender.lastname}  />
          </Item>
          ...
          <Item disabled   >
            <Input disabled
            multiline={true}
            defaultValue={this.state.olddeal.itemdata}
            placeholder={this.state.olddeal.itemdata}  />
            {objKeys.includes("itemdata")? <IconNB name="md-create" />:<Text></Text>}
          </Item>
          <Text style={styles.myFont1}>Сроки и порядок передачи имущества</Text>
          <Item disabled   >
            <Input disabled
            defaultValue={this.state.olddeal.giveawaydeadline}
            placeholder={this.state.olddeal.giveawaydeadline}  />
            {objKeys.includes("giveawaydeadline")? <IconNB name="md-create" />:<Text></Text>}
          </Item>
          <Text style={styles.myFont1}>Срок пользования имуществом</Text>
          <Item disabled   >
            <Input disabled
multiline={true}
            placeholder={this.dateFormat(this.state.olddeal.usedeadline)}  />
            {objKeys.includes("usedeadline")? <IconNB name="md-create" />:<Text></Text>}
          </Item>
          <Text style={styles.myFont1}>Условия о содержании имущества</Text>
          <Item disabled   >
            <Input disabled
            ...
            defaultValue={this.state.olddeal.keepcondition}
            placeholder={this.state.olddeal.keepcondition}  />
            {objKeys.includes("keepcondition")? <IconNB name="md-create" />:<Text></Text>}
          </Item>
          <Text style={styles.myFont1}>Условия об использовании имущества</Text>
          <Item disabled   >
            <Input disabled
multiline={true}
            defaultValue={this.state.olddeal.usecondition}
            placeholder={this.state.olddeal.usecondition}  />
            {objKeys.includes("usecondition")? <IconNB name="md-create" />:<Text></Text>}
          </Item>
    ...
    return (
      <Container>
        ...
        <Content padder>
          {this.state.seg === 1 &&
            <Content>
            <H2 style={styles.myFont}>Договор безвозмездного пользования имуществом</H2>
