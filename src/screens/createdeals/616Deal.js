...
const options = {
    ...
    workaddress:{
      label: "Адрес объекта при проведении работ по ремонту"
    },
    workdeadline: work_deadline,
    workprice:{
      label: "Цена работ (тенге)"
    },
    payday:{
      label: "Сроки и порядок оплаты"
    },
    workcheck:{
      label: "Порядок приема работ"
    },
    quantity:{
      label: "Качество работ (гарантия качества работ)"
    },
...
class NHListSeparator extends Component {
  ...
  getType(value) {
    if (value.Role === 'employee' && this.state.status1=='Физическое Лицо') {
      return t.struct({
        Role: Role,
        employer: t.enums(this.state.kontr_ids),
        workdescription: t.String,
    ...
    } else if (value.Role === 'employer' && this.state.status1=='Физическое Лицо' ) {
      return t.struct({
        Role: Role,
...
myNext(value){
	if(value.employer){
		...
		const formData = `deal616=${JSON.stringify(value)}&employer=${value.employer}&employee=${employee.sub}&duedate=${value.duedate}&workdeadline=${value.workdeadline}&lawid=${this.state.lawid}&status=${this.state.status}`;
		this.updateDeal(formData)
	} else{
		var employer = jwtDecode(this.state.myKey)
		const formData = `deal616=${JSON.stringify(value)}&employer=${employer.sub}&employee=${value.employee}&duedate=${value.duedate}&workdeadline=${value.workdeadline}&lawid=${this.state.lawid}&status=${this.state.status}`;
		this.updateDeal(formData)
	}
}
  onPress() {
	...
	render() {
        ...
        <Text>Подрядчик обязуется выполнить по заданию Заказчика определенную работу, а Заказчик обязуется оплатить такую работу на условиях, указанных в настоящем договоре.</Text>
