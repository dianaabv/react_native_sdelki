...
class SegmentNB extends Component {
  ...
  const formData = `deal506=${JSON.stringify(this.state.deal506)}&deal_id=${this.state.deal.deal_id}&duedate=${this.state.duedate}&workdeadline=${this.state.workdeadline}&lawid=${this.state.lawid}`
...
<Text style={styles.myFont1}>Одаряемый</Text>
<Item disabled>
  <Input disabled
  multiline={true}
  placeholder={this.state.deal.receiver.firstname+ ' '+ this.state.deal.receiver.lastname}  />
  <IconNB name="ios-information-circle" />
</Item>
<Text style={styles.myFont1}>Даритель</Text>
<Item disabled>
  <Input disabled
  multiline={true}
  placeholder={this.state.deal.presenter.firstname+ ' '+ this.state.deal.presenter.lastname}  />
    <IconNB name="ios-information-circle" />
</Item>

<Text style={styles.myFont1}>Наименование дара (вещи или имущественного права (требование) либо освобождения от имущественной обязанности)</Text>

  <TextInput underlineColorAndroid='transparent' style={[ !this.state.isRegular? styles.myInputBorder : styles.myInput]} numberOfLines = {3} editable={!this.state.isRegular}
  multiline={true}
   onChangeText={(text) => this.deal506(text, 'itemname')}
  defaultValue={this.state.deal.itemname}
  placeholder={this.state.deal.itemname}  />

<Text style={styles.myFont1}>Количество дара</Text>

  <TextInput style={[ !this.state.isRegular? styles.myInputBorder : styles.myInput]} numberOfLines = {3} editable={!this.state.isRegular}
  multiline={true}
  onChangeText={(text) => this.deal506(text, 'quantity')}
  defaultValue={this.state.deal.quantity}
  placeholder={this.state.deal.quantity}  />

<Text style={styles.myFont1}>Сроки и порядок передачи дара(момент перехода права собственности</Text>

    <TextInput style={[ !this.state.isRegular? styles.myInputBorder : styles.myInput]}  numberOfLines = {3} editable={!this.state.isRegular}
    multiline={true}
    onChangeText={(text) => this.deal506(text, 'deadline')}
...
<Text style={styles.myFont1}>Дополнительные условия (не обязательное ус-ие)</Text>
  ...
  <TextInput style={[ !this.state.isRegular? styles.myInputBorder : styles.myInput]} numberOfLines = {3} editable={!this.state.isRegular}
  multiline={true}
  onChangeText={(text) => this.deal506(text, 'additional')}
  ...
  <Spinner color="blue" />
        ...
        {(Object.keys(this.state.olddeal).length != 0)?(
          ...
          <Item disabled>
            <Input disabled
            multiline={true}
            placeholder={this.state.olddeal.receiver.firstname+ ' '+ this.state.olddeal.receiver.lastname}  />
            <IconNB name="ios-information-circle" />
          </Item>
          <Text style={styles.myFont1}>Даритель</Text>
          <Item disabled>
            <Input disabled
            multiline={true}
            placeholder={this.state.olddeal.presenter.firstname+ ' '+ this.state.olddeal.presenter.lastname}  />
              <IconNB name="ios-information-circle" />
          </Item>
